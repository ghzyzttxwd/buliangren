...
while (steps < 300) {
  ...
  if (event.action?.type === 'battle') {
    const battle = createBattle(event.action.enemies || [], state.party, state.martialArts, state);
    ...
    assert.equal(battle.allies[0].level, state.player.level, `battle player level mismatch: ${event.id}`);
    assert.equal(battle.allies[0].realm, state.player.realm, `battle player realm mismatch: ${event.id}`);
